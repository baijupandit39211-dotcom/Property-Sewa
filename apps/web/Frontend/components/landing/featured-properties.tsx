    ...
    <section className="bg-zinc-50 py-20 md:py-28">
      <div className="container mx-auto max-w-7xl px-4">
        ...
        <div className="text-center">
          <h2 className="text-3xl font-bold text-zinc-900 md:text-4xl">Featured Properties</h2>
          <p className="mt-3 text-zinc-500">Handpicked listings from the best locations, just for you.</p>
        </div>
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={`rounded-full px-5 py-2 text-sm font-medium transition-colors ${
                  activeTab === tab.value ? "bg-zinc-900 text-white" : "text-zinc-600 hover:bg-zinc-100"
                }`}
              >
                {tab.label}
              </button>
          ...
          {Array.from({ length: Math.min(totalPages, 3) }, (_, i) => i + 1).map((page) => (
            <button
              ...
              className={`flex h-9 w-9 items-center justify-center rounded-full text-sm font-medium transition-colors ${
                currentPage === page ? "bg-emerald-500 text-white" : "text-zinc-600 hover:bg-zinc-200"
              }`}
            >
              {page}
            </button>
          ...
          {totalPages > 3 && (
            <>
              ...
              <button
                ...
                className="flex h-9 w-9 items-center justify-center rounded-full text-sm font-medium text-zinc-600 hover:bg-zinc-200"
              >
                {totalPages}
              </button>
            </>
